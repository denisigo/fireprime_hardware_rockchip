...
ALOGD("output_route = %d input_route = %d",output_route, input_route);
    if (output_route)
...
static int start_output_stream(struct stream_out *out)
{
 ...
 //out->device |= AUDIO_DEVICE_OUT_SPEAKER;
//lkd
ALOGD("out->device & AUDIO_DEVICE_OUT_AUX_DIGITAL = 0x%x",out->device&AUDIO_DEVICE_OUT_AUX_DIGITAL);
ALOGD("out->device |AUDIO_DEVICE_OUT_SPEAKER = 0x%x",AUDIO_DEVICE_OUT_SPEAKER|AUDIO_DEVICE_OUT_SPEAKER);
ALOGD("out->device&(~AUDIO_DEVICE_OUT_AUX_DIGITAL) = 0x%x",out->device&(~AUDIO_DEVICE_OUT_AUX_DIGITAL));
...
ALOGD("+++ out->device = 0x%x", out->device);
    ...
    if (out->device & (AUDIO_DEVICE_OUT_SPEAKER |
        ...
        ALOGD("--- out->device ready = 0x%x",out->device & (AUDIO_DEVICE_OUT_SPEAKER |AUDIO_DEVICE_OUT_WIRED_HEADSET |AUDIO_DEVICE_OUT_WIRED_HEADPHONE |AUDIO_DEVICE_OUT_ALL_SCO));
...
ALOGD("+++ [1]last adev->out = %d", adev->out_device);
	out->device = 0x402;
    adev->out_device |= out->device;
ALOGD("+++ [2]last adev->out = %d", adev->out_device);
