...
// add by bunchen for setupUserCbMirrorFlip
// 0,90,270 will be fix if need
void CameraSOCAdapter::setupUserCbMirrorFlip(){
    LOGD("bunchen soc ori %d",gCamInfos[mCamId].facing_info.orientation);
       if(180==gCamInfos[mCamId].facing_info.orientation)
    {
       mRefEventNotifier->setUserCbMirrorFlipState(true,true);
    }else{
    }
}
