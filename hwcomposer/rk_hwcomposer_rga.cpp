    ...
    )
{
    ...
    rga_set_fds_offsets(&Rga_Request, 0, 0, 0,dst_ofset);
	//ALOGD("type=1,offset=%d",DstHandle->offset);
    ...
    LOGV("RGA dst:fd=%d,base=%p,dst_vir_w = %d, dst_vir_h = %d,dstLogical=%x,dstPhysical=%x,dstFormat=%d",dstFd,Context->membk_base[Context->membk_index],\
    ...
    )
{
    ...
    for (unsigned int i = 0; i < 1; i++)
    {
        ...
        LOGV("hwcLayerToWin%d: src[%d,%d,%d,%d], dst[%d,%d,%d,%d], src_ex[%d,%d] %d", Win,
            ...
            Src->exLeft,  Src->exRight, Src->exAddrOffset
            );
        
        info.xoffset = hwcMAX(srcRects[i].left - Src->exLeft, 0);
        info.yoffset = hwcMAX(srcRects[i].top , 0);
        ...
        info.yres = (srcRects[i].bottom - srcRects[i].top) ;
        info.xres_virtual = srcStride;
        info.yres_virtual = srcHeight;
	...
	if (GPU_FORMAT == HAL_PIXEL_FORMAT_YCrCb_NV12_VIDEO)
	{
	   if (Src->transform==HWC_TRANSFORM_ROT_90||Src->transform==HWC_TRANSFORM_ROT_270)
	   {
	      info.yres = (srcRects[i].right- srcRects[i].left) + (Src->exLeft + Src->exRight);
              info.xres = (srcRects[i].bottom - srcRects[i].top);
              info.xres_virtual = video_width;//info.xres ;//srcStride;
              info.yres_virtual = srcHeight * srcStride;//info.yres;//srcHeight ;
